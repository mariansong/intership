...
femData = read.csv("anova_diff_results(no).csv",stringsAsFactors = FALSE,check.names=F)
...
adjacency <- adjacency(WGCNA_matrix, power = 16)#
...
tiff(file="wgcna_anova_dynamicColors_plotDendroAndColors.tiff",width=17,height=15,units="cm", compression="lzw", res=600)
...
tiff(file="wgcna_anova_METree.tiff",width=34,height=10,units="cm", compression="lzw", res=600)#宽度设置比较长，方便观察
p1<-plot(METree,
...
tiff(file="wgcna_anova_mergedColors_plotDendroAndColors.tiff",width=17,height=17,units="cm", compression="lzw", res=600)
p1<-plotDendroAndColors(
  geneTree,
...
METree <- hclust(as.dist(MEDiss),method = "average")
tiff(file="wgcna_anova_METree.mergedColors.tiff",width=17,height=10,units="cm", compression="lzw", res=600)
p1<-plot(METree,
         main= "Clustering of module eigengenes",
         xlab= "", sub = "")
print(p1)
...
# modules heatmap
tiff(file="wgcna_anova_mergedColors.module_heatmap.tiff",width=17,height=17,units="cm", compression="lzw", res=600)
plotTOM<- dissTOM^7
p1<-TOMplot(plotTOM, geneTree, mergedColors, main="Network heatmapplot, all genes")
print(p1)
dev.off()

tiff(file="wgcna_anova_mergedColors.Eigengeneadjacency heatmap.tiff",width=17,height=21,units="cm",compression="lzw", res=600)
p2<-plotEigengeneNetworks(mergedMEs,"wgcna_anova_Eigengene adjacencyheatmap", marHeatmap = c(2,2,2,2),plotDendrograms = T,xLabelsAngle = 90) 
print(p2)
dev.off()
...
write.table(datout, "wgcna_anova_all_gene_module.xls", sep="\t", row.names=F,quote=F)
